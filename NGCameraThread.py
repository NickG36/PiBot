...
class ImageCapture(threading.Thread):
    ...
    def __init__(self, width, height, stream_processor):
        ...
        # Camera resolution defaults to the monitor's resolution,
...
class StreamProcessor(threading.Thread):
    ...
    def __init__(self, width, height, processing_cb=None, processing_cb_black = None, show_input=False):
        """
        ...
        print "______In init fn"
        self.event = threading.Event()
        self._exit_now = False
        self.max_processing_delay = 0.0  # Initialised for delay calculations
        self.min_processing_delay = 100.0  # Initialised for delay calculations
        self._frames_processed = 0
        self.width = width
        self.height = height
        self._processing_cb = processing_cb
        self._processing_cb_black = processing_cb_black
    ...
    self.comms_server = CommsServer.CommsServer();
        while not self._exit_now:
            # Wait for an image to be written to the stream
            # The wait() method takes an argument representing the
            # number of seconds to wait for the event before timing out.

	    print "Waiting to rx message"
	    #self.comms_server.wait()
	    was_green_req = self.comms_server.wait()
		    ...
		    print "Calling was green req......."
		    #if self.comms_server.was_green_req():
		    if was_green_req:
                      colour_amount = self.process_image(self.stream.array)
 		      print "...Gn amount %.2f" % colour_amount
		    else:
                      colour_amount = self.process_image_black(self.stream.array)
 		      print "...Black amount %.2f" % colour_amount

		    print "Sending response"
		    self.comms_server.send(colour_amount)
    ...
    def process_image(self, image):
        ...
        if self._processing_cb is not None:
            green_amount = self._processing_cb(image, self.width, self.height)
	    print "Green amount is %.2f" % green_amount
    	...
    	print "Processing Time taken: %f" % time_taken
    ...
    def process_image_black(self, image):
        """
        The main image processing function
        """
        # View the original image seen by the camera.
        ...
        if self._processing_cb_black is not None:
            black_amount = self._processing_cb_black(image, self.width, self.height)
	    print "Black amount is %.2f" % black_amount
